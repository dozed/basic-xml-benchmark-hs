...
-- Adapted from:
-- - https://github.com/travisbrown/xml-nodestream
-- - https://meta.plasm.us/posts/2013/05/29/lots-of-little-trees/
...
buildNode :: MonadThrow m => ConduitT Event o m (Maybe Node)
buildNode = do
  x <- CL.peek
  case x of
    ...
    Just (EventContent (ContentEntity e)) -> undefined -- lift . throwM $ InvalidEntity $ T.unpack e
    _ -> return Nothing
  where
    goE n as = do
      CL.drop 1
      ...
      if end == Just (EventEndElement n)
        ...
        else lift . throwM $ MissingEndElement n ((,) Nothing <$> end)

    ok x = CL.drop 1 >> return (Just x)

-- | A convenience function that ignores non-element nodes.
buildElement :: MonadThrow m => ConduitT Event o m (Maybe Element)
buildElement = do
  n <- buildNode
  case n of
...
buildElementCursor :: MonadThrow m => ConduitT Event o m (Maybe Cursor)
...
toAttrMap = M.fromList . map (second $ T.concat . map toText)
  where
    ...
    toText (ContentEntity e) = undefined -- throw $ InvalidEntity $ T.unpack e
